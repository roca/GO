...
func Add_user_with_PDF_And_message(messages []anthropic.MessageParam, message, base64String string) []anthropic.MessageParam {
	conversation := messages
	pdfBlock := anthropic.NewDocumentBlock(anthropic.Base64PDFSourceParam{
		Data:      base64String,
		MediaType: "application/pdf",
		Type:      "base64",
	})
	conversation = append(conversation,
		anthropic.NewUserMessage(pdfBlock),
		anthropic.NewUserMessage(anthropic.NewTextBlock(message)),
	)
	return conversation
}

func Add_user_with_Image_And_message(messages []anthropic.MessageParam, message, base64String string) []anthropic.MessageParam {
	conversation := messages
