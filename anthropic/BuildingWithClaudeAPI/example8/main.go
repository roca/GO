...
func main() {
	...
	userInput := "Is tea or coffee better at breakfast"
	conversation = add_user_message(conversation, userInput)

	assistant_prefilled_message := "Neither is very good and I would not recommend them because"
	conversation = add_assistant_message(conversation, assistant_prefilled_message)

	err := chat(conversation, 1.0, nil)
	if err != nil {
		panic(err.Error())
	}
