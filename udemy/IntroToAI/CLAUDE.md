...
- `-logic` flag is parsed in main.go but the propositional logic branch has `NewRobotWithLogic` commented out — `RobotWithLogic`, `ScanHouseWithLogic()`, and logic rules are fully implemented in `logic.go` but not yet called from main.go
